...
class Input extends LayerNode{
    ...
    computeShape(obj){         
        ...
        if(ret.code != 200)
            return ret; 
        return {code:200,w:this.out_shapes.w,h:this.out_shapes.h,channel:this.out_shapes.channel};
        /*
        try{
          ...
          this.out_shapes = {w:w,h:h,channel:input_channels};
          
          return {code:200,w:w,h:h,channel:input_channels};
        }
        catch(err){
            console.log(err);
...
class Padding extends LayerNode{
    ...
    computeShape(obj){         
        ...
        if(ret.code != 200)
            return ret; 
        try{
            let wout = w + 2 * parseInt(this.padding);
            let hout = h + 2 * parseInt(this.padding);
